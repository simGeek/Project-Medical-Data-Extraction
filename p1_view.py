...
def p1_uploadextract(request):
    if request.method == 'POST':
        ...
        if file.name.endswith('.pdf'):
            ...
            file_path = os.path.join(settings.MEDIA_ROOT, 'pdf_files', filename)                                                                                 file_path = os.path.join(settings.MEDIA_ROOT, 'pdf_files', filename)
            ...
            return JsonResponse(data, safe=False)                                                                               else:
